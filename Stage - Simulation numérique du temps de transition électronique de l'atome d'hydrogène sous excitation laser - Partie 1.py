            ...
            if(Eee<-0.01):
                if abs(Eee +0.5) > 0.1:
                    ...
                    if(dra==1):
                        ...
                        times_laser = np.linspace(t0, tsse.x*3, 15000)
                    ...
                    if(dra==1):
                        ...
                        # 2. Trouver t_arrivee : stabilisation sur n=2 ou n=3 (10 pas consécutifs)
                        t_arrivee_n2 = None
                        t_arrivee_n3 = None
                        count_n2 = 0
                        count_n3 = 0
                        ...
                        for i in range(idx_depart, len(E_array)):
                            ...
                            if E_array[i] >= E_n2:
                                count_n2 += 1
                                if count_n2 >= 10 and t_arrivee_n2 is None:
                                    t_arrivee_n2 = t_as[i - 10]
                            else:
                                count_n2 = 0
                            ...
                            if E_array[i] >= E_n3:
                                count_n3 += 1
                                if count_n3 >= 10 and t_arrivee_n3 is None:
                                    t_arrivee_n3 = t_as[i - 10]
                            else:
                                count_n3 = 0
                        
                        
                        # Vérifier que l'énergie finale est proche d'un palier (stabilisation réelle)
                        E_final = E_array[-1]
                        tolerance = 0.2  # eV
                        
                        if t_arrivee_n2 is not None:
                            if abs(E_final - E_n2) > tolerance:
                                t_arrivee_n2 = None  # faux positif, on annule
                        
                        if t_arrivee_n3 is not None:
                            if abs(E_final - E_n3) > tolerance:
                                t_arrivee_n3 = None  # faux positif, on annule
                                                
                        # 3. Affichage
                        if t_arrivee_n2 is not None or t_arrivee_n3 is not None:
                            ...
                            # Figure 4 : énergie en fonction du temps
                            plt.figure(4)
                            plt.xlim(0,200)
...
num_samples, initial_precision, precision_lower_bound, precision_factor = 40000, 0.15, 0.05, 2
